...
const CONTRACT_NAME = "dev-1638317169154-49485974812571" || "near-challenge-10.testnet";
...
window.addEventListener("DOMContentLoaded", event => {
  ...
  const humanDonationAmount = document.getElementById("human-donation-amount");
  ...
  donateToCat.onclick = async () => {
    try {
      console.log({
        fund: "Cat"
      }, BOATLOAD_OF_GAS, (0, _big.default)(catnewDonationAmount.value || "0").times(10 ** 24).toFixed());
  ...
  donateToDog.onclick = async () => {
    try {
      console.log({
        fund: "Dog"
      }, BOATLOAD_OF_GAS, (0, _big.default)(dognewDonationAmount.value || "0").times(10 ** 24).toFixed());
      await window.contract.donate({
        fund: "Dog"
      }, BOATLOAD_OF_GAS, (0, _big.default)(dognewDonationAmount.value || "0").times(10 ** 24).toFixed());
      updateCandidates();
  ...
  donateToHuman.onclick = async () => {
    try {
      console.log({
        fund: "Human"
      }, BOATLOAD_OF_GAS, (0, _big.default)(humannewDonationAmount.value || "0").times(10 ** 24).toFixed());
      await window.contract.donate({
        fund: "Human"
      }, BOATLOAD_OF_GAS, (0, _big.default)(humannewDonationAmount.value || "0").times(10 ** 24).toFixed());
      updateCandidates();
  ...
  function signedInFlow() {
    ...
    document.getElementById("balance").innerText = (window.accountBalance.available / 10 ** 24).toFixed(5) + " NEAR";
    document.querySelectorAll(".logged-in-display").forEach(el => {
  ...
  async function updateCandidates() {
    try {
      ...
      for (let i = 0; i < pets.length; i += 1) {
        if (pets[i].key == "Cat") {
          catDonationAmount.innerText = (Math.round(pets[i].value.donations * 10000) / 10000).toString();
          total += pets[i].value.donations;
        }

        if (pets[i].key == "Dog") {
          dogDonationAmount.innerText = (Math.round(pets[i].value.donations * 10000) / 10000).toString();
          total += pets[i].value.donations;
        }

        if (pets[i].key == "Human") {
          humanDonationAmount.innerText = (Math.round(pets[i].value.donations * 10000) / 10000).toString();
          total += pets[i].value.donations;
        }
      }

      document.getElementById("total-donations").innerText = total.toString();
      updateLogs();
    } catch (e) {
      console.log(e);
      alert("Something went wrong! " + "Maybe you need to sign out and back in? " + "Check your browser console for more info.");
...
},{"big.js":"../node_modules/big.js/big.js","regenerator-runtime/runtime":"../node_modules/regenerator-runtime/runtime.js","./utils":"utils.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
